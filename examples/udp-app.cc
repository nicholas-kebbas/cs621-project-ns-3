...
//
  ...
  // Install udp server node into the app
  ApplicationContainer apps = server.Install (udpNodes.Get (1));
...
//
  ...
  apps = appClient.Install (udpNodes.Get (0));
  ...
  csma.EnableAsciiAll (ascii.CreateFileStream ("udp-client-server.tr"));
  csma.EnablePcapAll ("udp-client-server", false);
  pointToPoint.EnablePcapAll ("udp-client-p2p", false);
